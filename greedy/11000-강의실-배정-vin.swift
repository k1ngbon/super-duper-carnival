...
typealias Lecture = (start: Int, end: Int)
let N = Int(readLine()!)!
var lectures = [Lecture]()
var heap = Heap<Lecture>{ t1, t2 in
...
heap.insert(lectures.removeFirst())

for i in 0..<N-1 {
    let lecture = lectures[i]
    let lastLecture = heap.peek!
    
    if lecture.start >= lastLecture.end {
        heap.remove()
    }
    heap.insert(lecture)
}
